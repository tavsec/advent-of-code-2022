...
func (s *Ship) executeInstruction(count, from, to int) {
	...
	elements := s.Crates[from][lastElIndex-count+1 : lastElIndex+1]
	s.Crates[to] = append(s.Crates[to], elements...)

	s.Crates[from] = s.Crates[from][:lastElIndex-count+1]
}
