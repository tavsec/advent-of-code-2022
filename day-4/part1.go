...
func fullyContain(id1 ID, id2 ID) bool {
	return (id1.Start <= id2.Start && id1.End >= id2.End) || (id2.Start <= id1.Start && id2.End >= id1.End)
}
...
func main() {
	...
	for fileScanner.Scan() {
		...
		if fullyContain(id1, id2) {
			counter++
		}
	}

	fmt.Println("Number of IDs that contains each other: " + fmt.Sprint(counter))
}
